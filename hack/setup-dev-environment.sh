...
REGISTRY_PORT="5000"
CLUSTER_NAME="datadog-controller-dev"
...
kubectl create namespace datadog-controller-system || true
kubectl create configmap registry-certs \
  --from-file=ca.crt="${CERTS_DIR}/domain.crt" \
  -n datadog-controller-system || true

# Build the controller image
echo "Building controller image..."
make docker-build IMG="datadog-controller:dev-local"

# Load the image into kind
echo "Loading image into Kind cluster..."
kind load docker-image datadog-controller:dev-local --name "${CLUSTER_NAME}"
...
metadata:
  ...
  namespace: datadog-controller-system
data:
  copy-certs.sh: |
    #!/bin/sh
    mkdir -p /work-dir
...
make deploy IMG="datadog-controller:dev-local"
...
kubectl patch deployment -n datadog-controller-system datadog-controller-controller-manager --patch '
spec:
  template:
    spec:
      initContainers:
